...
class HistoricalWindow:
    ...
    def validate(self) -> None:
        ...
        if self.resolution != DEFAULT_RESOLUTION:
            raise HistoricalWindowError("first version supports only 1h resolution")
...
def aggregate_hourly_trade_bars(
    ...
    *,
    ...
    """Aggregate Deribit trade rows into hourly OHLCV bars."""
    ...
    for trade in trades:
        ...
        bucket_start = timestamp - (timestamp % HOUR_MS)
        buckets.setdefault(bucket_start, []).append((timestamp, price, amount))

    bars: list[HourlyTradeBar] = []
    for bucket_start in sorted(buckets):
        rows = sorted(buckets[bucket_start], key=lambda item: item[0])
        ...
        bars.append(
            HourlyTradeBar(
                ...
                volume_contracts=sum(row[2] for row in rows),
            )
        )
    return tuple(bars)
