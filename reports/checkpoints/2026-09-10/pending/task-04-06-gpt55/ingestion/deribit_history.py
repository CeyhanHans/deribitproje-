    ...
    *,
    ...
    for attempt in range(1, max_retries + 1):
        if deadline_monotonic is not None and time.monotonic() >= deadline_monotonic:
            raise DeribitHistoryError("Request deadline exhausted before attempt")
        try:
            effective_timeout = timeout
            if deadline_monotonic is not None:
                effective_timeout = max(0.001, min(timeout, deadline_monotonic - time.monotonic()))
            with urlopen(request, timeout=effective_timeout) as response:
                ...
                while True:
                    ...
                    if not chunk:
                        break
                    byte_size += len(chunk)
                    if max_response_bytes is not None and byte_size > max_response_bytes:
                        raise DeribitHistoryError(
                            f"Deribit response exceeded max_response_bytes={max_response_bytes}"
                        )
                    if deadline_monotonic is not None and time.monotonic() >= deadline_monotonic:
                        raise DeribitHistoryError("Request deadline exhausted while reading response body")
                    chunks.append(chunk)
            ...
            if exc.code == 429:
                ...
                if attempt < max_retries:
                    if deadline_monotonic is not None:
                        retry_seconds = min(retry_seconds, max(0.0, deadline_monotonic - time.monotonic()))
                    if retry_seconds > 0:
                ...
                if attempt < max_retries:
                    ...
                    if deadline_monotonic is not None:
                        sleep_for = min(sleep_for, max(0.0, deadline_monotonic - time.monotonic()))
                    if sleep_for > 0:
                        sleeper(sleep_for)
                    continue
