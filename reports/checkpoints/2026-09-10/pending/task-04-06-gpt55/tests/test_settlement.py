...
class SettlementTests(unittest.TestCase):
    ...
    def test_exact_expiry_boundary(self):
        ...
        # Off by 1 millisecond is an instrument/request identity mismatch, not a valid missing quote.
        with self.assertRaises(SettlementError):
            provider.get("BTC-29MAR24-60000-C", expiry_ms - 1)
        with self.assertRaises(SettlementError):
            provider.get("BTC-29MAR24-60000-C", expiry_ms + 1)
    ...
    def test_fee_schedule_historical_verification_status(self):
        # The compatibility alias keeps the import name, but source_updated_at is not effective_at evidence.
        t_current = 1787000000000
        schedule_current = get_fee_schedule_for_timestamp(t_current)
        self.assertEqual(schedule_current.verification_status, VerificationStatus.CONFIGURED_UNVERIFIED)
        self.assertIn("effective_at=unverified", schedule_current.source_reference)
    ...
    def test_wrong_index_and_malformed_instrument_rejected(self):
        expiry_ms = 1735286400000  # 2024-12-27 08:00 UTC
        with self.assertRaises(SettlementError):
            DeliveryPriceRecord(
        ...
        provider = DeribitSettlementProvider([DeliveryPriceRecord(expiry_ms, Decimal("100000"))])
        with self.assertRaises(SettlementError):
            provider.get("not-an-option", expiry_ms)

    def test_instrument_expiry_must_match_requested_expiry(self):
        expiry_ms = 1735286400000
        provider = DeribitSettlementProvider([DeliveryPriceRecord(expiry_ms, Decimal("100000"))])
        with self.assertRaisesRegex(SettlementError, "does not match requested expiry_ms"):
            provider.get("BTC-26DEC24-90000-C", expiry_ms)

    def test_network_fetch_paginates_until_target_expiry(self):
        expiry_ms = 1735286400000
        page_1 = (DeliveryPriceRecord(1735200000000, Decimal("99000")),)
        page_2 = (DeliveryPriceRecord(expiry_ms, Decimal("100000")),)
        provider = DeribitSettlementProvider(allow_network_fetch=True, delivery_page_count=1, max_delivery_pages=3)

        with patch("ingestion.settlement.fetch_official_delivery_prices", side_effect=[page_1, page_2]) as mocked:
            obs = provider.get("BTC-27DEC24-90000-C", expiry_ms)

        self.assertIsNotNone(obs)
        self.assertEqual(obs.settlement_price_usd, Decimal("100000"))
        self.assertEqual(mocked.call_count, 2)

    def test_network_fetch_error_is_not_reported_as_missing_price(self):
        expiry_ms = 1735286400000
        provider = DeribitSettlementProvider(allow_network_fetch=True)

        with patch("ingestion.settlement.fetch_official_delivery_prices", side_effect=SettlementError("api down")):
            with self.assertRaisesRegex(SettlementError, "api down"):
                provider.get("BTC-27DEC24-90000-C", expiry_ms)

    def test_daily_delivery_fee_uses_configured_daily_rate(self):
        schedule = FeeSchedule(
            "custom-daily-rate",
            0,
            None,
            delivery_fee_rate_daily=Decimal("0.001"),
        )
        fee = schedule.compute_delivery_fee(
            quantity=Decimal("1.0"),
            settlement_price_btc=Decimal("1.0"),
            is_daily=True,
        )
