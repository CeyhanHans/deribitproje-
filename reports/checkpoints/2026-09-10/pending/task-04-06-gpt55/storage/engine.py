...
class TradeStorageEngine:
    ...
    def store_trades(
        ...
        *,
        ...
        self._validate_completion_upgrade(manifest, dest_path)
        self.index.register_archive(manifest)
        return self.index.find_exact_archive(instrument_name, start_ms, end_ms) or manifest
    ...
    def find_cache_hit(
        ...
        """
        hit = self.index.find_complete_cache_hit(instrument_name, start_ms, end_ms)
        if hit is None:
            return None
        archive_file = validate_safe_path(hit.archive_relpath, self.root_dir)
        self.reader.verify_checksum(archive_file, hit.sha256)
        return hit
    ...
    def stream_trades(
        ...
        for arch in matching_archives:
            archive_file = validate_safe_path(arch.archive_relpath, self.root_dir)
            if not archive_file.exists():
                raise FileNotFoundError(f"Archive file {archive_file} referenced by index does not exist")
        ...
        *,
        ...
        for arch in archives:
            ...
            if not src_file.exists():
                raise FileNotFoundError(f"Archive file {src_file} referenced by index does not exist")
    ...
    def _validate_completion_upgrade(
        self,
        incoming_manifest: ArchiveManifest,
        incoming_archive_path: Path,
    ) -> None:
        existing = self.index.find_exact_archive(
            incoming_manifest.instrument_name,
            incoming_manifest.start_ms,
            incoming_manifest.end_ms,
        )
        if existing is None:
            return
        if existing.sha256 == incoming_manifest.sha256:
            return
        if not (
            existing.status == ArchiveStatus.INCOMPLETE.value
            and incoming_manifest.status == ArchiveStatus.COMPLETE.value
        ):
            return

        existing_file = validate_safe_path(existing.archive_relpath, self.root_dir)
        with sqlite3.connect(":memory:") as conn:
            conn.execute(
                ...
                CREATE TABLE incoming (
                    ...
                    PRIMARY KEY (trade_seq, trade_id)
                )
                """
            )
            for tick in self.reader.stream_ticks(
                incoming_archive_path,
                expected_sha256=incoming_manifest.sha256,
            ):
                conn.execute(
                    """
                    INSERT OR REPLACE INTO incoming (
                        trade_seq, trade_id, instrument_name, timestamp_ms, price_btc, quantity_contracts
                    ) VALUES (?, ?, ?, ?, ?, ?)
                    """,
                    (
                        tick.trade_seq,
                        ...
                        tick.timestamp_ms,
                        str(tick.price_btc),
                        str(tick.quantity_contracts),
                    ),
                )

            cur = conn.cursor()
            for old in self.reader.stream_ticks(existing_file, expected_sha256=existing.sha256):
                cur.execute(
                    """
                    SELECT instrument_name, timestamp_ms, price_btc, quantity_contracts
                    FROM incoming
                    WHERE trade_seq = ? AND trade_id = ?
                    """,
                    (old.trade_seq, old.trade_id),
                )
                row = cur.fetchone()
                if row is None:
                    raise StorageConflictError(
                        "Completion upgrade drops an existing trade identity "
                        f"trade_seq={old.trade_seq}, trade_id={old.trade_id}"
                    )
                new_inst, new_ts, new_price, new_qty = row
                if (
                    old.instrument_name != new_inst
                    or old.timestamp_ms != int(new_ts)
                    or old.price_btc != Decimal(str(new_price))
                    or old.quantity_contracts != Decimal(str(new_qty))
                ):
                    raise StorageConflictError(
                        "Completion upgrade conflicts with existing economic trade content "
                        f"for trade_seq={old.trade_seq}, trade_id={old.trade_id}"
                    )
